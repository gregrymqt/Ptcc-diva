...
/* --------------------------------------------------
   ...
   Cria a estrutura HTML do drawer e coloca no
   final do body. É chamado uma única vez.
   -------------------------------------------------- */

/* Injeta o HTML da gaveta do carrinho no final do body.
   Se a gaveta já existir, não faz nada (evita duplicar). */
function injetarHTMLdaGaveta() {
  // Verifica se a gaveta já foi criada antes
  ...
  var html =
    ...
    '<div id="cart-drawer" class="cart-drawer">' +
      ...
      '<div id="cart-body" class="cart-body">' +
        '<!-- Os itens do carrinho são colocados aqui pelo JavaScript -->' +
      '</div>' +
  ...
  // Insere o HTML antes do fechamento do body
...
/* --------------------------------------------------
   ...
   Converte um número (ex: 49.9) para o formato
   de moeda brasileira (ex: R$ 49,90).
   -------------------------------------------------- */
...
/* --------------------------------------------------
   ...
   Lê o carrinho e monta o HTML de cada item
   dentro do corpo (#cart-body) da gaveta.
   -------------------------------------------------- */

/* Atualiza o conteúdo da gaveta com os itens atuais do carrinho.
   É chamada toda vez que o carrinho muda. */
export function renderCartItems() {
  var cartBody    = document.getElementById("cart-body");
  var subtotalEl  = document.getElementById("cart-subtotal-value");

  // Se a gaveta ainda não está na página, não faz nada
  ...
  // Atualiza o valor do subtotal no rodapé da gaveta
  ...
  if (carrinho.length === 0) {
    ...
    // O botão "Continuar Comprando" fecha a gaveta
  ...
  // Após colocar os itens na tela, registra os eventos dos botões
  registrarEventosDosItens();
}

/* Registra os eventos de clique nos botões de quantidade
   e remoção de cada item da gaveta. */
function registrarEventosDosItens() {
  ...
  for (var i = 0; i < botoesMenos.length; i++) {
    botoesMenos[i].addEventListener("click", function() {
      ...
      // Pega a quantidade atual lendo o texto do elemento ao lado
      ...
      if (quantidadeAtual > 1) {
        ...
        // Se a quantidade for 1, pergunta se quer remover o item
  ...
  for (var j = 0; j < botoesMais.length; j++) {
    botoesMais[j].addEventListener("click", function() {
      ...
      // Pega a quantidade atual lendo o texto do elemento anterior
  ...
  // Botões de remover item (Excluir)
...
/* --------------------------------------------------
   ...
   Controla a visibilidade da gaveta do carrinho.
   -------------------------------------------------- */

/* Abre a gaveta lateral do carrinho.
   Garante que o HTML existe, exibe a gaveta e
...
export function openCartDrawer() {
  ...
  // Adiciona a classe "open" para tornar a gaveta visível via CSS
  overlay.classList.add("open");
  gaveta.classList.add("open");

  // Conecta o overlay e o botão X ao fechamento da gaveta
  overlay.onclick        = closeCartDrawer;
  if (btnFechar) {
    btnFechar.onclick    = closeCartDrawer;
  }

  // Carrega e exibe os itens do carrinho dentro da gaveta
  renderCartItems();
}

/* Fecha a gaveta lateral do carrinho
   removendo a classe "open" dos elementos. */
...
/* --------------------------------------------------
   ...
   o HTML da gaveta é injetado de forma invisível,
   pronto para ser aberto quando necessário.
   -------------------------------------------------- */
